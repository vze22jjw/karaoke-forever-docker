...
if [ "$VESRION" == "latest" ]; then
    ...
    --build-arg CURRENT_VERSION=$VERSION --build-arg CURRENT_VERSION=$PORT -t "$DOCKER_IMAGE_NAME":latest
else
    ### MAKE A LATEST TAG AS WELL AS VERSION TAG
    docker build -f /volume1/VOLUME1/DATA_BACKUP/DOCKER_BUILDS/karaoke-forever-docker/Dockerfile . \
    --build-arg CURRENT_VERSION=$VERSION --build-arg CURRENT_VERSION=$PORT \
